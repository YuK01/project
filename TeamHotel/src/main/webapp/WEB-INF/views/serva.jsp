...
<h1 style="text-align:center; font-weight:bold; font-size:40px; letter-spacing:6px;">예약관리</h1>
   ...
   <div class="div_body1"  style="margin-left:1px;">
...
.on('click','#selBook option',function(){
   ...
   $('#start_dtt').val(ar[2]);
   $('#end_dtt').val(ar[3]);
   let type_code=parseInt(ar[4]);
   let room_code=parseInt(ar[5]);
   let howmany=parseInt(ar[6]);
   $('#type_codee').val(type_code);
  
   $.ajax({url:"/hotel/roomname",
         data:{type:$('#type_codee').val()},
         method:"GET",
         datatype:"json",
         success:function(txt){
      ...
      $('#room_name').val(room_code);
      $('#howmanyy').val(howmany);
      console.log(howmany);
      $('#howmuch').val(ar[7]);
      $('#mobile').val(ar[8]);
...
function booklist(){
   ...
   for(i=0; i<txt.length; i++){
      let str='<option value='+txt[i]['book_id']+'>'+txt[i]['name']+' '+txt[i]['booker']+' '+
            txt[i]['start_dt']+'~'+txt[i]['end_dt']+' '+txt[i]['type_code']+' '+txt[i]['room_code']+' '+txt[i]['howmany']+' '+
            txt[i]['howmuch']+' '+txt[i]['mobile']+'</option>';
...
function clean(){
   $('#book_id,#start_dtt,#end_dtt,#type_codee,#room_name,#mobile,#howmuch').val('');
   $('#booker').val('');
   $('#howmanyy,#howmany,#type_code').val('');
   $('#room_name').val('');
   $('#selRoom').empty();
   $('#start_dt,#end_dt').val('');
//    $('#room_name').empty();
